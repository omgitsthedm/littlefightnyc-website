...
## Technical Decisions
> ⚠️ **STALE (pre-2026 static era).** The site below described a hand-built static HTML/CSS/JS site. **The live site is now a React 19 + Vite + TypeScript SPA under `app/`, prerendered for SEO via `scripts/prerender-seo.mjs`, auto-deployed from GitHub `main`.** See **`AGENTS.md` → "Tech Stack & Architecture (current)"** for the authoritative stack, and the dated `## 2026-07-…` entries at the top of this file for current decisions. The bullets below are retained only as history of the retired static build.
- Static HTML/CSS/JS site, deployed on Netlify. *(retired — see note above)*
