...
- **VERA removed from studioProjects** (`/studio/vera/` prerender; 404 live). At the time it was described on-site as a "listing intelligence pipeline" powering Local Search, which the product did not do. **Superseded 2026-07-29:** VERA now exists and ships — a live NYC apartment-hunt system at `/vera/`, served as a hand-written static app from `app/public/vera/` with its own brand, terms, and privacy pages, backed by a nightly pipeline whose public feed proxies to `vera-pipeline.netlify.app` (see `app/public/_redirects`). It is linked from the site and is not a studio project entry. The removal above was correct for what VERA was in that release; do not act on the "killed / never shipped" description.
