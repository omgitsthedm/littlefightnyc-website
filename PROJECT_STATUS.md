...
>
> **Last updated:** 2026-07-16 · **Updated by:** Claude Code (LiFi NYC) · **Stage:** Live flagship marketing site (Apple-tier bar)
...
- **Status:** ✅ Live and healthy. Continuously elevated across many 2026 waves (type system, perf splits, WCAG AA, premium interaction, SEO-beast schema, orange browser-chrome).
- **Live URL:** https://littlefightnyc.com/ — **200 OK** (verified 2026-07-16).
- **Git:** fully synced — **0 uncommitted, 0 unpushed**. Last commit `c5b50b7` (2026-07-15) — "docs: record completed orange browser-chrome set (ecc67d6)".
...
## 4. What's done
Mature React 19 + TypeScript + Vite 7 SPA (React Router 7) with build-time SEO prerendering (`scripts/prerender-seo.mjs` writes ~102–127 static route `index.html` files with full JSON-LD: FAQPage, BreadcrumbList, LocalBusiness/ProfessionalService, DefinedTerm, Article, HowTo). Delivered scope includes: full content build-out (services, case studies with honest metrics, answer guides, neighborhood/area hubs, glossary, journal, industries, studio), the **Axiom Momentum** design system, the **"Boxing Poster" type system** (Oswald-700 condensed display + Barlow-400 body, self-hosted via @fontsource), motion/route-transition system, command palette (Cmd/Ctrl-K), perf splits (site.ts + journal body chunking), WCAG 2.1 AA pass (0 axe violations across pages), security headers, and complete orange browser-chrome. Lighthouse: Home ~92 / Services ~90 perf, BP + SEO 100, CLS 0, TBT 0; squirrelscan 83/B (ceiling is by-design, not defects).
