...
const available = new Set([...document.querySelectorAll('article[data-id]')].map(card => card.dataset.id));
let saved = [];
try { const value = JSON.parse(localStorage.getItem(storageKey) || '[]'); if (Array.isArray(value)) saved = [...new Set(value.filter(id => available.has(id)))]; } catch { /* Optional local preferences. */ }
let onlySaved = false;
function update() {
 ...
 const visible = document.querySelector('#visible-count'); if (visible) visible.textContent = (onlySaved ? saved.length : available.size) + ' designs';
