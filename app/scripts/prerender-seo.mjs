...
function legalBlock(page) {
  ...
  return `
    ...
    <p>If you choose “Allow visits + Meta,” the Meta Pixel and Meta’s web Conversions API connection share public page views, contact clicks, inquiry starts, and confirmed form completions with Meta. It uses browser identifiers such as _fbp and _fbc, device information, IP address, and approved referral labels to measure which social visits lead to inquiries. This does not tell us the identity of every visitor or person who clicks.</p>
    <p>We do not send form answers, names, email addresses, phone numbers, or private report links to Meta. Automatic advanced matching and automatic event collection are off. Meta may associate activity with its accounts and use it for personalized content and ads, under its <a href="https://www.facebook.com/privacy/policy/" target="_blank" rel="noreferrer">privacy policy</a>. You can manage Meta’s use in its “Activity from other businesses” settings.</p>
    <h2>Google Ads measurement</h2>
    <p>“Allow visits + Google Ads” lets Google Analytics share inquiry completion events with our linked Google Ads account. Approved campaign labels and Google click identifiers help connect an ad click with a later inquiry. Google may use first-party advertising cookies such as _gcl and _gac for this measurement. We do not send your form answers or contact details, upload customer lists, or enable Google ad personalization or remarketing.</p>
    <h2>Cookies, pixels, and opt-outs</h2>
    <p>Analytics is off by default for a first-time visitor. Google Analytics loads after a visit-counting choice. Google Ads and Meta each need their own explicit permission; allowing one does not grant permission to the other. An existing permission for the other service stays as you set it. “Allow visit counting” turns both advertising choices off. “Essential only” keeps all optional measurement off and the site still works. Global Privacy Control keeps both advertising choices off. Withdrawing advertising permission stops its measurement and removes the corresponding first-party advertising cookies; it does not erase data already received by a service. You can change your choice at any time using the footer.</p>
...
function zhSnapshot() {
  ...
  return `
    <div class="lf-seo" lang="zh">
      ...
      <footer>Little Fight NYC · 纽约 · 始于2021 · 依然有人接电话 · <a href="/">查看完整英文网站</a><div class="lf-seo__credit">${lfCreditMarkup}</div></footer>
    </div>`;
}
  ...
  return `
    <div class="lf-seo" lang="es">
      ...
      <footer>
        ...
        <div class="lf-seo__credit">${lfCreditMarkup}</div>
      </footer>
    </div>`;
}
...
function snapshot(page) {
  ...
  const inlineStyles = `
    ...
    .lf-seo .lf-seo__home-backdrop img { display: block; width: 100%; height: 100%; object-fit: cover; object-position: 48% 42%; }
    .lf-seo .lf-seo__home-backdrop picture { display: block; height: 100%; }
    .lf-seo .lf-seo__home-backdrop::after { position: absolute; inset: 0; content: ""; background: linear-gradient(90deg, rgb(5 5 7 / 82%) 0%, rgb(5 5 7 / 56%) 40%, rgb(5 5 7 / 10%) 72%, rgb(5 5 7 / 0%) 100%), linear-gradient(180deg, rgb(5 5 7 / 30%) 0%, rgb(5 5 7 / 0%) 26%, rgb(5 5 7 / 28%) 60%, rgb(5 5 7 / 100%) 100%); }
    ...
    .lf-seo__credit { --lfc-size: 11px; display: flex; margin-top: 18px; padding-top: 14px; border-top: 1px solid #27272A; color: #8A8A94; }
    ${lfCreditCss}
  `;
