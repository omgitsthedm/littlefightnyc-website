...
function legalBlock(page) {
  ...
  return `
    ...
    <p>Analytics is off by default for a first-time visitor. Google Analytics loads after “Allow visit counting” or “Allow visits + Meta.” Meta loads only after the second choice. “Essential only” keeps both off and the site still works. Global Privacy Control keeps Meta off. Changing to visit counting only or essential only stops Meta events and removes its first-party cookies; it does not erase data already received by Meta. You can change your choice at any time using the footer.</p>
