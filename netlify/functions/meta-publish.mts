...
const META_GRAPH_REQUEST_TIMEOUT_MS = 15_000;
...
async function publishToInstagram(
  ...
  const container = await graphRequest(
    ...
    `${META_INSTAGRAM_ACCOUNT_ID}/media`,
    {
      ...
      ...(payload.alt_text ? { alt_text: payload.alt_text } : {}),
    },
    config,
  );
...
export default async function metaPublish(
  ...
  if (contentLength) {
    ...
    if (!Number.isSafeInteger(declaredLength) || declaredLength > META_MAX_BODY_BYTES) {
  ...
  try {
    ...
    return privateJson({ error: "Publishing payload could not be read." }, 503);
  }
  ...
  if (payload.publish) {
    const liveEnabled =
      Netlify.env.get("META_PUBLISH_LIVE_ENABLED")?.trim().toLowerCase() === "true";
    if (!context.deploy.published || !liveEnabled) {
      return privateJson({ error: "Live publishing is disabled for this deploy." }, 403);
    }
    if (payload.platform === "instagram") {
      return privateJson(
        { error: "Live Instagram publishing is disabled pending durable container polling." },
        503,
      );
    }
  }
